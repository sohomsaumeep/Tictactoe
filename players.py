...
class SmartComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        # first we want to check if the previous move had a winner
        # this is our base case
        a = 0
        if state.current_winner == other_player:
            # we should return position and score, to keep track of score
            #for minimax to work
        ...
        for possible_move in state.available_moves():
            ...
            # step 2: recurse using miniax to simulate a game after making that move
            sim_score = self.minimax(state, other_player) #now, we alternate players

            # step 3: undo that move, so we can try the next move in future iteration
